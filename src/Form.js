...
const nava = useNavigate()
  ...
  const [input1Value,setValue1]=useState(state.edit  ? state.edit[0]?.name : '')
  const [input2Value,setValue2]=useState(state.edit ? state.edit[0]?.desp : "")
    ...
    <section>
      ...
      <form onSubmit={ buttonSubmit} className='col-12 form border text-center  shadow py-3 bg-secondary'>
       ...
       <input type='checkbox' onChange={checkbox} ></input><br></br>
        ...
        <input type='submit' className='button bg-danger text-light mt-3' onClick ={()=> dispatch({type:'push',payload:arr})}></input>   
      </form>
      </div>
      </div>
      </div>
